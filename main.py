...
ВАЖНО:
- Для "сегодня" и "завтра" говори ТОЛЬКО об одном конкретном дне
- Не упоминай недели, месяцы, периоды, если это гороскоп на один день
- Используй слова "сегодня" или "завтра", а не "период"
- Будь максимально конкретным и привязанным к одному дню
- Не описывай расчёты.
...
1. Короткое персональное вступление (1–2 предложения) с упоминанием дня/периода {period_header}

2. Энергия дня (для сегодня/завтра) или Энергия периода (для недели/месяца):
(1 абзац: общий настрой, внутреннее состояние, ритм дня/периода с привязкой к датам)

3. Ключевые сферы дня/периода:
...
4. Возможные вызовы дня (для сегодня/завтра) или Возможные риски периода (для недели/месяца): 
(конкретно и практично, с привязкой к датам)

5. Совет от чисел  
(прикладной, применимый в реальной жизни)

6. Число удачи на {period_display} + его значение (1–2 предложения)

7. Короткое итоговое резюме с акцентом на период {period_display}

Запрещено использовать:
- для "сегодня" и "завтра" упоминать о "периодах", "неделях", "месяцах" кроме указанных
- английские слова
- транслитерацию
- слова today, period и т.п.

ДЛЯ НЕДЕЛИ/МЕСЯЦА:
- Разбивай на фазы (первая/вторая половина недели, декады месяца)
- Указывай конкретные даты для рекомендаций

ИСПОЛЬЗУЙ ПРАВИЛЬНЫЕ СЛОВА:
- Для "сегодня": "энергия дня", "вызовы дня", "сегодняшний день"
- Для "завтра": "энергия завтрашнего дня", "завтра", "в этот день"
- Для "недели": "энергия недели", "в течение недели", "по дням недели"
- Для "месяца": "энергия месяца", "в течение месяца", "по декадам"

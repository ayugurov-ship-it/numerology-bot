...
_LANG_RULE = (
    ...
    "Обращение ТОЛЬКО на «вы/ваш/вам». "
    "ЗАПРЕЩЕНЫ местоимения «он», «она», «его», «её» — пиши ТОЛЬКО от второго лица. "
...
Будь дипломатичным, подчеркивай сильные стороны пары. {_LANG_RULE}""",
    ...
    "horoscope": f"""Ты — профессиональный редактор ежедневных и периодических астрологических прогнозов.
Интерпретируй только переданные рассчитанные астрологические и нумерологические данные.
Положения планет, Луна, фаза, аспекты и переходы являются исходными фактами и не должны изменяться.
Не добавляй несуществующие транзиты. Каждый раздел должен быть связан с конкретным фактором из входных данных.
Тон: живой, конкретный, содержательный, без шаблонных фраз.
Это развлекательная интерпретация, а не научное предсказание будущего. {_LANG_RULE}""",
...
Особенно проверяй аспекты, дома, ASC/MC, градусы и неопределённость Луны.
Отдельно отмечай логические противоречия и повторяющиеся общие фразы.
Если факт не подтверждается картой, считай это ошибкой.
Отвечай кратко и строго в формате VERDICT: PASS или VERDICT: FAIL, затем ISSUES.
{_LANG_RULE}"""
}
...
# (start, end, name_nominative, emoji, element, name_locative)
ZODIAC_SIGNS = [
    ((3, 21), (4, 19), "Овен", "♈", "огонь", "Овне"),
    ((4, 20), (5, 20), "Телец", "♉", "земля", "Тельце"),
    ((5, 21), (6, 20), "Близнецы", "♊", "воздух", "Близнецах"),
    ((6, 21), (7, 22), "Рак", "♋", "вода", "Раке"),
    ((7, 23), (8, 22), "Лев", "♌", "огонь", "Льве"),
    ((8, 23), (9, 22), "Дева", "♍", "земля", "Деве"),
    ((9, 23), (10, 22), "Весы", "♎", "воздух", "Весах"),
    ((10, 23), (11, 21), "Скорпион", "♏", "вода", "Скорпионе"),
    ((11, 22), (12, 21), "Стрелец", "♐", "огонь", "Стрельце"),
    ((12, 22), (12, 31), "Козерог", "♑", "земля", "Козероге"),
    ((1, 1), (1, 19), "Козерог", "♑", "земля", "Козероге"),
    ((1, 20), (2, 18), "Водолей", "♒", "воздух", "Водолее"),
    ((2, 19), (3, 20), "Рыбы", "♓", "вода", "Рыбах"),
]
...
def get_zodiac_sign(date_str: str) -> Optional[dict]:
    ...
    try:
        ...
        for (m1, d1), (m2, d2), name, emoji, element, locative in ZODIAC_SIGNS:
            if m1 == m2:
                if month == m1 and d1 <= day <= d2:
                    return {"name": name, "emoji": emoji, "element": element, "locative": locative}
            elif m1 < m2:
...
async def natal_chart_handler(m: Message, date_str: str, birth_time: str = None):
    ...
    await m.answer("🌌 Составляю ваш натальный портрет...")

    prompt = f"""
Составь натальный портрет по имеющимся данным. Это НЕ расчёт полной натальной карты. Обращайся на «вы» (НИКОГДА не «он», «она», «его», «её»).
...
Вместо этого описывай только те характеристики, которые можно связать с достоверным знаком Солнца, стихией и числом пути. Не создавай впечатление, что рассчитаны дома, аспекты или положения других планет.
...
💼 — Реализация и карьера: стиль работы, принятие решений, отношение к ответственности и долгим задачам. Не перечисляй случайные профессии. 2–3 предложения.
...
Если время рождения известно, полная карта дополнительно рассчитает:
⬆️ Асцендент
🏠 дома

Если время рождения неизвестно — это не проблема: карта всё равно строится по дате и месту рождения, просто без Асцендента и домов.

В обоих вариантах рассчитываются:
🌙 Луна
☿ Меркурий
♀ Венера
♂ Марс
...
async def generate_full_natal_chart(message: Message, user_id: int, paid: bool = False, test_mode: bool = False):
    ...
    time_known = bool(data.get("birth_time"))
    prompt = f"""
...
Данные рождения:
{data["date"]} {data.get("birth_time") or "время неизвестно"}, {data["place"]}, часовой пояс {data["timezone"]}.
...
РЕЖИМ КАРТЫ:
{"Точное время известно. Можно интерпретировать Асцендент, MC, дома и положения планет по домам." if time_known else "Время рождения неизвестно. НЕ интерпретируй Асцендент, MC, дома или положения планет по домам. Не выдавай положение Луны как абсолютно точное, если в данных указана неопределённость."}

СТРУКТУРА:
1. Ключевой портрет — Солнце, Луна и, только если рассчитан, Асцендент.
2. Личное мышление и общение — Меркурий.
3. Любовь и отношения — Венера, Марс и Луна; 7-й дом использовать только если он рассчитан.
4. Работа, деньги и реализация — 2-й, 6-й и 10-й дома только если они рассчитаны; иначе анализируй планеты и аспекты без домов.
...
8. Выделенные жизненные темы — дома использовать только при наличии.
...
"""
    ...
    angle_lines = ""
    if chart.get("angles"):
        if chart["angles"].get("ascendant"):
            angle_lines += "\n\n*Асцендент:* " + chart["angles"]["ascendant"]["formatted"]
        if chart["angles"].get("mc"):
...
Карта дня — это ежедневный расклад: общая энергетика дня и как она влияет
на человека с учётом его знака зодиака и числа пути.

ВАЖНО: ты НЕ имеешь доступа к эфемеридам. НЕ ВЫДУМЫВАЙ конкретные положения планет
(«Луна в Овне», «Марс в квадрате к Солнцу» и т.п.). Описывай энергию дня
через нумерологию даты и общие характеристики знака {zodiac_name}.

Формат ответа — ТОЛЬКО эмодзи-разделители, БЕЗ текстовых заголовков, обращение на «вы»:

🌅 — Энергия дня: общий настрой, ритм, темп сегодняшнего дня для {zodiac_name}. 2 предложения.

⚡ — На чём сосредоточиться: главная задача или возможность сегодня. 2 предложения.

💼 — Работа и дела: что делать, чего избегать. 2 предложения.
...
ЗАПРЕЩЕНО:
- «он», «она», «его», «её» — ТОЛЬКО «вы»
- выдумывать положения планет по знакам
- текстовые заголовки разделов
- англицизмы и транслитерации
- «вселенная», «карма», «потоки»
- общие фразы и абстрактная философия
"""
...
if __name__ == "__main__":
    ...
    logger.info("• Натальный портрет по дате рождения")

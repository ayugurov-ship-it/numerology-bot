...
def patch():
    ...
    source = replace_once(source, r'@router\.message\(lambda m: m\.text == "🔮 Мой профиль"\).*?@router\.message\(lambda m: m\.text == "💞 Совместимость"\)', PROFILE, "profile")
    source = replace_once(source, r'@router\.callback_query\(lambda c: c\.data\.startswith\("horoscope_"\)\).*?@router\.message\(lambda m: m\.text == "🔢 Нумерология"\)', HOROSCOPE, "horoscope")
    source = replace_once(source, r'@router\.message\(lambda m: m\.text == "🔢 Нумерология"\).*?@router\.message\(lambda m: m\.text == "🌌 Натальная карта"\)', NUMEROLOGY, "numerology")
    source = replace_once(source, r'@router\.message\(lambda m: m\.text == "🌌 Натальная карта"\).*?@router\.message\(lambda m: m\.text == "✨ Карта дня"\)', NATAL, "natal")
    source = replace_once(source, r'@router\.message\(lambda m: m\.text == "✨ Карта дня"\).*?@router\.message\(lambda m: m\.text == "👑 Админ-панель"\)', DAILY, "daily")
...
if __name__ == "__main__":
    changed = patch()
    print("calendar patch applied" if changed else "calendar patch already present")
